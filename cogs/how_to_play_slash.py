...
class HTP_view(discord.ui.View):
...
class How_to_play(slash.Cog):
	def __init__(self, bot):
		self.bot = bot


	# guild_id=879153063036858428
	@slash.slash_command(name="how-to-play")
	async def how_to_play(self, ctx : slash.Context):
		...
		embed_2 = discord.Embed(title="How To Play - Goal", description="In order to win, a player must get four discs in their color in a row. Whoever does it first is the winner.", color=int("5261f8", 16))
		...
		while True:
			...
			page_embed.set_footer(text=f"{page} / 3", icon_url=ctx.author.avatar.url)
			...
			await ctx.send(embed=page_embed, view=view, ephemeral=True)
			await view.wait()

			if view.quit: break

def setup(bot):
	bot.add_cog(How_to_play(bot))