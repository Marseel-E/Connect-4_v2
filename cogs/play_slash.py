from discord import Embed, ButtonStyle, Member, SelectOption, Interaction
...
from database import get_user, Game, redis, fetch_games
...
class Empty_view(View):
	def __init__(self):
		super().__init__()


class Play_ask_view(View):
...
class Slash_play(Cog):
	...
	@command()
	@guilds(test_server)
	@describe(member="Your opponent", bet="Your bet")
	async def play(self, interaction: Interaction, member: Member, bet: int = 0):
		if (member.id == interaction.user.id):
			await interaction.response.send_message("No- just no.. you cant play with yourself.", ephemeral=True)
			return
		...
		if (player.playing or opponent.playing):
			await interaction.response.send_message("You're playing another game", ephemeral=True)
			return

		bet = 0 if (bet < 0) else bet

		view = Play_ask_view(member)
		msg = await interaction.response.send_message(f"{member.mention}, `{interaction.user}` invited you to a game for {bet} :coin:", view=view)
		await view.wait()

		if view.value == "refuse":
			await msg.delete()
			await interaction.response.send_message(content=f"{interaction.user.mention}, `{member}` refused your invitation", ephemeral=True)
			return
		...
		if (player.coins < bet) or (opponent.coins < bet):
			await msg.delete()
			await interaction.response.send_message(content="You or your opponent cant afford the bet", ephemeral=True)
			return
		...
		while not game.finished:
			...
			while True:
				...
				await interaction.response.send_message(":warning: Choose another move :warning:", ephemeral=True)
...
async def setup(bot):
	await bot.add_cog(Slash_play(bot))